...
func TestGetResourceTypesWithFallback(t *testing.T) {
	...
	expectedClusterScoped := len(k8skit.K8sClusterScopedResourceTypes)
	expectedNamespaced := len(k8skit.K8sNamespacedResourceTypes)
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedResourceTypes: expectedClusterScoped,
		},
		{
			name:                  "namespaced only",
			includeClusterScoped:  false,
			includeNamespaced:     true,
			expectedResourceTypes: expectedNamespaced,
		},
		{
			name:                  "both scoped types",
			includeClusterScoped:  true,
			includeNamespaced:     true,
			expectedResourceTypes: expectedClusterScoped + expectedNamespaced,
		},
		{
