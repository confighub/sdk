...
var kubernetesConfigFactory = func(kubeContext string) (*rest.Config, error) {
...
func NewFluxRendererWorker() *FluxRendererWorker {
	// FIXME: Note that this doesn't call NewKubernetesBridgeWorker(), because FluxRendererWorker
	// inlines KubernetesBridgeWorker rather than storing a pointer.
	return &FluxRendererWorker{}
}
...
func findFluxResource(data []byte) (kind, apiVersion, name, namespace string, err error) {
	parsed, err := ParseDocumentsWithResources(data)
	if err != nil {
...
func (w *FluxRendererWorker) applyAuthoritative(wctx api.BridgeWorkerContext, payload api.BridgeWorkerPayload) error {
	...
	result, err := RenderFlux(wctx.Context(), input)
	if err != nil {
		wctx.SendStatus(common.NewActionResult(
			api.ActionStatusFailed,
			api.ActionResultApplyFailed,
...
func (w *FluxRendererWorker) refreshNonAuthoritative(wctx api.BridgeWorkerContext, payload api.BridgeWorkerPayload) error {
	...
	renderResult, err := RenderFlux(wctx.Context(), input)
	if err != nil {
		wctx.SendStatus(common.NewActionResult(
			api.ActionStatusFailed,
			api.ActionResultRefreshFailed,
...
func lookupArtifactSource(ctx context.Context, k8sClient client.Client, parsed *ParsedDocuments) (ArtifactSource, error) {
	...
	return ArtifactSource{}, fmt.Errorf("no HelmRelease or Kustomization found in input")
}
...
func lookupHelmReleaseSource(ctx context.Context, k8sClient client.Client, hr *unstructured.Unstructured, helmCharts map[string]*unstructured.Unstructured) (ArtifactSource, error) {
	spec, ok := hr.Object["spec"].(map[string]interface{})
	if !ok {
		return ArtifactSource{}, fmt.Errorf("HelmRelease has no spec")
	}
	...
	return ArtifactSource{}, fmt.Errorf("HelmRelease has neither chartRef nor chart spec")
}

// lookupChartRefSource looks up artifact from a chartRef (HelmChart or OCIRepository).
func lookupChartRefSource(ctx context.Context, k8sClient client.Client, hr *unstructured.Unstructured, chartRef map[string]interface{}) (ArtifactSource, error) {
	...
	default:
		return ArtifactSource{}, fmt.Errorf("unsupported chartRef kind: %s", kind)
	}
}
...
func lookupImplicitChartSource(ctx context.Context, k8sClient client.Client, hr *unstructured.Unstructured, chartSpec map[string]interface{}, helmCharts map[string]*unstructured.Unstructured) (ArtifactSource, error) {
	innerSpec, ok := chartSpec["spec"].(map[string]interface{})
	if !ok {
		return ArtifactSource{}, fmt.Errorf("chart spec has no inner spec")
	}

	sourceRef, ok := innerSpec["sourceRef"].(map[string]interface{})
	if !ok {
		return ArtifactSource{}, fmt.Errorf("chart spec has no sourceRef")
	}
	...
	if err := createOrUpdateHelmChart(ctx, k8sClient, helmChart); err != nil {
		return ArtifactSource{}, fmt.Errorf("failed to create HelmChart: %w", err)
	}
...
func waitForHelmChartArtifact(ctx context.Context, k8sClient client.Client, namespace, name string) (ArtifactSource, error) {
	...
	if err != nil {
		return ArtifactSource{}, fmt.Errorf("timeout waiting for HelmChart %s/%s artifact: %w", namespace, name, err)
	}
...
func lookupKustomizationSource(ctx context.Context, k8sClient client.Client, ks *unstructured.Unstructured) (ArtifactSource, error) {
	spec, ok := ks.Object["spec"].(map[string]interface{})
	if !ok {
		return ArtifactSource{}, fmt.Errorf("Kustomization has no spec")
	}

	sourceRef, ok := spec["sourceRef"].(map[string]interface{})
	if !ok {
		return ArtifactSource{}, fmt.Errorf("Kustomization has no sourceRef")
	}
	...
	default:
		return ArtifactSource{}, fmt.Errorf("unsupported Kustomization sourceRef kind: %s", kind)
	}
}

// lookupHelmChartArtifact fetches a HelmChart and extracts the artifact URL and digest.
func lookupHelmChartArtifact(ctx context.Context, k8sClient client.Client, namespace, name string) (ArtifactSource, error) {
...
func lookupGitRepositoryArtifact(ctx context.Context, k8sClient client.Client, namespace, name string) (ArtifactSource, error) {
...
func lookupOCIRepositoryArtifact(ctx context.Context, k8sClient client.Client, namespace, name string) (ArtifactSource, error) {
...
func lookupBucketArtifact(ctx context.Context, k8sClient client.Client, namespace, name string) (ArtifactSource, error) {
...
func lookupSourceArtifact(ctx context.Context, k8sClient client.Client, apiVersion, kind, namespace, name string) (ArtifactSource, error) {
	...
	if err := k8sClient.Get(ctx, key, source); err != nil {
		return ArtifactSource{}, fmt.Errorf("failed to get %s %s/%s: %w", kind, namespace, name, err)
	}

	// Extract artifact from status
	status, ok := source.Object["status"].(map[string]interface{})
	if !ok {
		return ArtifactSource{}, fmt.Errorf("%s %s/%s has no status", kind, namespace, name)
	}

	artifact, ok := status["artifact"].(map[string]interface{})
	if !ok {
		return ArtifactSource{}, fmt.Errorf("%s %s/%s has no artifact in status (not ready?)", kind, namespace, name)
	}
	...
	if url == "" {
		return ArtifactSource{}, fmt.Errorf("%s %s/%s artifact has no URL", kind, namespace, name)
	}
