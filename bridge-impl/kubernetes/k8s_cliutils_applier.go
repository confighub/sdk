...
applyDrainLoop:
	for {
		...
		case e, ok := <-eventChannel:
			...
			// Log errors from apply events so failures are not silent
			switch e.Type {
			case event.ErrorType:
				log.Log.Error(e.ErrorEvent.Err, "❌ Apply error event")
				applyErrors = append(applyErrors, e.ErrorEvent.Err.Error())
			case event.ApplyType:
				if e.ApplyEvent.Status == event.ApplyFailed && e.ApplyEvent.Error != nil {
					...
					applyErrors = append(applyErrors, fmt.Sprintf("%s: %s", e.ApplyEvent.Identifier, e.ApplyEvent.Error))
				}
	...
	// Return error if any resources failed to apply
...
drainLoop:
	for {
		...
		case e, ok := <-eventChannel:
			...
			// Log errors from destroy events so failures are not silent
			switch e.Type {
			case event.ErrorType:
				log.Log.Error(e.ErrorEvent.Err, "❌ Destroy error event")
				destroyErrors = append(destroyErrors, e.ErrorEvent.Err.Error())
			case event.DeleteType:
				if e.DeleteEvent.Status == event.DeleteFailed && e.DeleteEvent.Error != nil {
					...
					destroyErrors = append(destroyErrors, fmt.Sprintf("%s: %s", e.DeleteEvent.Identifier, e.DeleteEvent.Error))
				}
			case event.ApplyType:
				if e.ApplyEvent.Status == event.ApplyFailed && e.ApplyEvent.Error != nil {
	...
	// Return error if any resources failed to destroy
