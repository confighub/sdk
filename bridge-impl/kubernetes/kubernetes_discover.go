...
func isAdministrativeResource(kind, name string) bool {
	...
	adminResourceKinds := []string{
		...
		"Endpoints",                 // Endpoints generated by the Service endpoint controller
		"EndpointSlice",             // Endpoints generated by the Service endpoint controller
	}
	...
	adminResourceNames := []string{
		...
		"endpoints",                  // Endpoints generated by the Service endpoint controller
		"endpointslices",             // Endpoints generated by the Service endpoint controller
	}

	return slices.Contains(adminResourceKinds, kind) || slices.Contains(adminResourceNames, name)
}

// TODO: also ignore the kubernetes Service and kube-root-ca.crt ConfigMap and default ServiceAccount
...
func NewImportConfigFromRequest(request *goclientnew.ImportRequest) *ImportConfig {
	config := &ImportConfig{
		...
		IncludeSystem:  cubapi.ParseBoolOption(request.Options, "include_system", false),
		IncludeCustom:  cubapi.ParseBoolOption(request.Options, "include_custom", false),
		IncludeCluster: cubapi.ParseBoolOption(request.Options, "include_cluster", false),
		IncludeOwned:   cubapi.ParseBoolOption(request.Options, "include_owned", false),
	}
...
func (rd *ResourceDiscovery) applyGenericFilters(resources []*unstructured.Unstructured) []*unstructured.Unstructured {
	...
	for _, resource := range resources {
		...
		if !rd.config.IncludeOwned && resource.GetOwnerReferences() != nil {
			continue
		}
