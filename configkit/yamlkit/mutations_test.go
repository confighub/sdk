...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Compute first diff
			firstMutations, err := yamlkit.ComputeMutations(baseParsed, firstParsed, 1, k8skit.K8sResourceProvider)
			require.NoError(t, err)

			// Compute second diff (from first change to second change)
			secondMutations, err := yamlkit.ComputeMutations(firstParsed, secondParsed, 2, k8skit.K8sResourceProvider)
			require.NoError(t, err)
...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			sourceMutations, err := yamlkit.ComputeMutations(baseParsed, sourceParsed, 1, k8skit.K8sResourceProvider)
			require.NoError(t, err)

			// Compute target diff (base -> targetChange)
			targetMutations, err := yamlkit.ComputeMutations(baseParsed, targetParsed, 2, k8skit.K8sResourceProvider)
			require.NoError(t, err)
...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := yamlkit.PatchMutations(parsed, tt.predicates, tt.mutations, k8skit.K8sResourceProvider)
			require.NoError(t, err)
...
`
		...
		sourceMutations, err := yamlkit.ComputeMutations(baseParsed, sourceEndParsed, 1, k8skit.K8sResourceProvider)
		require.NoError(t, err)
		...
		result, err := yamlkit.PatchMutations(targetParsed, nil, finalMutations, k8skit.K8sResourceProvider)
		require.NoError(t, err)
...
`
		...
		_, hasImage := targetMutations[0].PathMutationMap[api.ResolvedPath("spec.template.spec.containers.0.image")]
		...
		patchedResult, err := yamlkit.PatchMutations(targetParsed, nil, result, k8skit.K8sResourceProvider)
		require.NoError(t, err)
...
`
		...
		resourcesPath := api.ResolvedPath("spec.template.spec.containers.0.resources")
		...
		require.Len(t, targetMutations, 1)
		cpuPath := api.ResolvedPath("spec.template.spec.containers.0.resources.limits.cpu")
...
`
		...
		cpuPath := api.ResolvedPath("spec.template.spec.containers.0.resources.limits.cpu")
		memoryPath := api.ResolvedPath("spec.template.spec.containers.0.resources.limits.memory")
