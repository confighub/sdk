...
func NewArgoCDOCIWorker(workerID, workerSecret string) *ArgoCDOCIWorker {
	return &ArgoCDOCIWorker{
		KubernetesBridgeWorker: *NewKubernetesBridgeWorker(),
		workerID:               workerID,
...
func generateArgoCDRepoCreds(host, namespace, workerID, workerSecret string, isHTTP bool) ([]byte, error) {
	normalizedHost := k8skit.K8sResourceProvider.NormalizeName(host)
...
func (w *ArgoCDOCIWorker) WatchForApply(wctx api.BridgeWorkerContext, payload api.BridgeWorkerPayload) error {
	...
	k8sClient, _, err := kubernetesClientFactory(params.KubeContext)
	if err != nil {
		return lib.SafeSendStatus(wctx, newActionResult(
			api.ActionStatusFailed,
			api.ActionResultApplyWaitFailed,
	...
	for {
		...
		progressStatus := newActionResult(
			api.ActionStatusProgressing,
			api.ActionResultNone,
		...
		if overallSync == argoCDSyncStatusSynced && overallHealth == argoCDHealthStatusHealthy &&
			...
			liveStateYAML, yamlErr := objectsToYAML([]*unstructured.Unstructured{liveApp})
			if yamlErr != nil {
				return lib.SafeSendStatus(wctx, newActionResult(
					api.ActionStatusFailed,
					api.ActionResultApplyWaitFailed,
			...
			cleanedObjects := cleanupObjects([]*unstructured.Unstructured{liveApp})
			liveDataYAML, yamlErr := objectsToYAML(cleanedObjects)
			if yamlErr != nil {
				return lib.SafeSendStatus(wctx, newActionResult(
					api.ActionStatusFailed,
					api.ActionResultApplyWaitFailed,
...
func (w *ArgoCDOCIWorker) Refresh(wctx api.BridgeWorkerContext, payload api.BridgeWorkerPayload) error {
	...
	cleanedApp := cleanupObjects([]*unstructured.Unstructured{liveApp})
	liveDataYAML, yamlErr := objectsToYAML(cleanedApp)
	if yamlErr != nil {
		return lib.SafeSendStatus(wctx, newActionResult(
			api.ActionStatusFailed,
			api.ActionResultRefreshFailed,
	...
	if len(downstreamObjects) > 0 {
		liveDownstream, liveErr := getLiveObjects(wctx.Context(), k8sClient, downstreamObjects, false, true)
		if liveErr != nil {
			log.Log.Error(liveErr, "Failed to fetch downstream resources")
		} else if len(liveDownstream) > 0 {
			cleanedDownstream := extraCleanupObjects(liveDownstream)
			downstreamYAML, downstreamYAMLErr := objectsToYAML(cleanedDownstream)
			if downstreamYAMLErr != nil {
				...
				patched, drifted, diffErr := yamlkit.DiffPatchWithOptions(baseDataWithoutComments, []byte(downstreamYAML), originalData, k8skit.K8sResourceProvider, false)
				if diffErr != nil {
	...
	result := newActionResult(api.ActionStatusCompleted, resultType, message)
...
func filterOutSecrets(data []byte) []byte {
	...
	result, err := objectsToYAML(filtered)
	if err != nil {
