...
import (
	...
	fluxrenderer "github.com/confighub/sdk/bridge-worker/impl/flux-renderer"
...
func (w *FluxRendererWorker) Apply(wctx api.BridgeWorkerContext, payload api.BridgeWorkerPayload) error {
	...
	// Create Kubernetes client to look up source resources
	...
	// This can be stored out of order because rendering is so fast. Don't do this
	// until we prevent out-of-order UnitEvents.
	// if err := wctx.SendStatus(newActionResult(
	// 	api.ActionStatusProgressing,
	// 	api.ActionResultNone,
	// 	"Rendering Flux resources...",
	// )); err != nil {
	// 	return err
	// }
...
func (w *FluxRendererWorker) WatchForApply(wctx api.BridgeWorkerContext, payload api.BridgeWorkerPayload) error {
	// FluxRenderer doesn't apply to a cluster, so there's nothing to watch
	return nil
}
...
func lookupOCIRepositoryArtifact(ctx context.Context, k8sClient client.Client, namespace, name string) (fluxrenderer.ArtifactSource, error) {
	return lookupSourceArtifact(ctx, k8sClient, "source.toolkit.fluxcd.io/v1beta2", "OCIRepository", namespace, name)
}
...
func lookupBucketArtifact(ctx context.Context, k8sClient client.Client, namespace, name string) (fluxrenderer.ArtifactSource, error) {
	return lookupSourceArtifact(ctx, k8sClient, "source.toolkit.fluxcd.io/v1beta2", "Bucket", namespace, name)
}
