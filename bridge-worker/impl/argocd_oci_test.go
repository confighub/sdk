...
func TestArgoCDOCIWorker_Apply_TransformError(t *testing.T) {
	...
	setupMockSendStatusContains(t, mockCtx, api.ActionStatusFailed, api.ActionResultApplyFailed, "cannot infer OCI host")
...
func TestArgoCDOCIWorker_Import_NotSupported(t *testing.T) {
	mockCtx := setupMockContext(t)
	setupMockSendStatusContains(t, mockCtx, api.ActionStatusFailed, api.ActionResultImportFailed, "Import not supported")
...
func TestArgoCDOCIWorker_Destroy_TransformError(t *testing.T) {
	mockCtx := setupMockContext(t)
	mockCtx.On("GetServerURL").Return("").Maybe()
	setupMockSendStatusContains(t, mockCtx, api.ActionStatusFailed, api.ActionResultDestroyFailed, "cannot infer OCI host")
...
func TestArgoCDOCIWorker_WatchForApply_TransformError(t *testing.T) {
	mockCtx := setupMockContext(t)
	mockCtx.On("GetServerURL").Return("").Maybe()
	setupMockSendStatusContains(t, mockCtx, api.ActionStatusFailed, api.ActionResultApplyWaitFailed, "cannot infer OCI host")
...
func TestArgoCDOCIWorker_WatchForDestroy_TransformError(t *testing.T) {
	mockCtx := setupMockContext(t)
	mockCtx.On("GetServerURL").Return("").Maybe()
	setupMockSendStatusContains(t, mockCtx, api.ActionStatusFailed, api.ActionResultDestroyWaitFailed, "cannot infer OCI host")
...
func TestArgoCDOCIWorker_Refresh_NoDrift(t *testing.T) {
	...
	// 1. Progress status
	setupMockSendStatus(t, mockCtx, api.ActionStatusProgressing, api.ActionResultNone, "Retrieving ArgoCD Application state...")
...
func TestArgoCDOCIWorker_Refresh_TransformError(t *testing.T) {
	mockCtx := setupMockContext(t)
	mockCtx.On("GetServerURL").Return("").Maybe()
	setupMockSendStatusContains(t, mockCtx, api.ActionStatusFailed, api.ActionResultRefreshFailed, "cannot infer OCI host")
...
func TestNewArgoCDOCIWorker_ApplierTypeInitialized(t *testing.T) {
	...
	assert.Equal(t, CLIUtilsSSA, worker.applierType, "NewArgoCDOCIWorker should initialize applierType via NewKubernetesBridgeWorker")
...
func TestGetLiveObjects_SkipNotFound(t *testing.T) {
	t.Run("fetches found resources", func(t *testing.T) {
		...
		result, err := getLiveObjects(context.Background(), mockClient, expected, false, true)
		assert.NoError(t, err)
	...
	t.Run("returns NotFound error when skipNotFound=false", func(t *testing.T) {
		...
		expected := []*unstructured.Unstructured{testConfigMap.DeepCopy()}
		result, err := getLiveObjects(context.Background(), mockClient, expected, false, false)
		assert.Error(t, err)
		assert.Nil(t, result)
	})
	...
	t.Run("empty expected objects", func(t *testing.T) {
		mockClient := new(MockK8sClient)
		result, err := getLiveObjects(context.Background(), mockClient, nil, false, true)
		assert.NoError(t, err)
		assert.Nil(t, result)
	})
}
