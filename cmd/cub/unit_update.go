...
func init() {
	...
	unitUpdateCmd.Flags().StringVar(&whereMutation, "where-mutation", "", "where expression selecting this unit's mutations whose paths the merge must not overwrite, on top of the stored path protection (only used with --merge-source). On a --merge-source Self revert, where the stored protection is not consulted, it is the whole of what the revert leaves alone")
	unitUpdateCmd.Flags().StringVar(&filterMutation, "filter-mutation", "", "filter selecting this unit's mutations whose paths the merge must not overwrite, on top of the stored path protection (only used with --merge-source)")
