...
func TestK8sVetCEL_BoolPass(t *testing.T) {
	...
	_, output, err := generic.GenericFnVetCEL(testResourceProvider, nil, nil, docs, args, k8sCELEnvOpts()...)
	require.NoError(t, err)
...
func TestK8sGetCEL_ExtractReplicas(t *testing.T) {
	...
	_, output, err := generic.GenericFnGetCEL(testResourceProvider, nil, nil, docs, args, k8sCELEnvOpts()...)
	require.NoError(t, err)
...
func TestK8sSetCEL_SetReplicas(t *testing.T) {
	...
	newDocs, _, err := generic.GenericFnSetCEL(testResourceProvider, nil, docs, args, nil, k8sCELEnvOpts()...)
	require.NoError(t, err)
