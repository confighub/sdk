...
`ResolveAssociativePaths` supports direct index references with `.?<key>=@<index>`, which selects the array element at the given index regardless of the key value. For example, `spec.template.spec.containers.?name=@0.image` resolves to `spec.template.spec.containers.0.image`.
...
```
spec.template.spec.containers.?name=nginx;@0.env.?name=DATABASE_URL;@1.value
```

This means "the container named `nginx` (currently at index 0), its env var named `DATABASE_URL` (currently at index 1), the `value` field". `ResolveAssociativePaths` resolves these paths by trying the key=value match first, then falling back to the positional index if the key is not found.

`ResolveAssociativeSegments` resolves `?key=value;@index` paths against a document, converting them to numeric-only paths suitable for gaby operations. `StripAssociativeSegments` extracts just the numeric indices without consulting a document.

The merge keys for Kubernetes resource types are defined in `k8skit.MergeKeyForPath`. The `ResourceProvider` interface includes a `MergeKeyForPath(resourceType, path)` method that returns the merge key field name for a given array path, or false if the array is not associative.
...
type FunctionInvocationRequest struct {
	...
	WhereResource       string                 `json:",omitempty" description:"If non-empty, restrict which resources functions operate on using path expressions"`
...
The `WhereResource` field filters which resources within a Unit the functions operate on. See [Resource filtering with WhereResource](#resource-filtering-with-whereresource) below.
...
The response `Outputs` field is a map from `OutputType` to the corresponding output data as embedded JSON. Multiple outputs of compatible types (e.g., `AttributeValueList`, `ResourceInfoList`, `ValidationResultList`) are combined automatically.
...
### Resource filtering with WhereResource

The `WhereResource` field on `FunctionInvocationRequest` restricts which resources within a Unit's configuration data a function operates on. It accepts relational expressions that are evaluated per-resource.

Two kinds of paths are supported:

1. **ConfigHub metadata paths** — these reference resource metadata extracted by the `ResourceProvider`:
   - `ConfigHub.ResourceName` — fully qualified resource name (e.g., `default/my-app` for Kubernetes)
   - `ConfigHub.ResourceNameWithoutScope` — resource name without namespace/scope
   - `ConfigHub.ResourceType` — fully qualified resource type (e.g., `apps/v1/Deployment`)
   - `ConfigHub.ResourceCategory` — resource category (e.g., `Resource`, `AppConfig`)

2. **Resource field paths** — these reference fields within the YAML document itself (e.g., `kind`, `spec.replicas`, `metadata.namespace`). The value is looked up from the resource's YAML document using `yamlkit.YamlSafePathGetValueAnyType`.

Multiple expressions can be combined with `AND`. Examples:

```
--where-resource "ConfigHub.ResourceType = 'apps/v1/Deployment'"
--where-resource "ConfigHub.ResourceType = 'apps/v1/Deployment' AND metadata.namespace = 'production'"
...
fctl do --where-resource "ConfigHub.ResourceType = 'apps/v1/Deployment'" all-in-one.yaml "MyUnit" get-resources
```
...
The following configuration formats are supported, identified by their ToolchainType:

- Kubernetes/YAML (`ToolchainKubernetesYAML`)
- AppConfig/Properties — Java Properties files (`ToolchainAppConfigProperties`)
